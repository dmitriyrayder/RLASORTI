...
Исправленная версия с корректными расчетами
"""
...
class BusinessMetricsCalculator:
    """
    Калькулятор бизнес-метрик (ИСПРАВЛЕННЫЙ).
    """
    ...
    def calculate_metrics(
        ...
        """
        Расчет метрик для текущего ассортимента (ИСПРАВЛЕНО).
        """
        if sku_df.empty:
            return AssortmentMetrics(
                total_gmv=0, total_profit=0, total_qty=0,
                avg_margin_pct=0, num_skus=0, num_segments=0,
                out_of_stock_cost=0, roi=0, inventory_cost=0, turnover_rate=0
            )
        ...
        num_skus = len(sku_df)
        num_segments = sku_df['Segment'].nunique() if 'Segment' in sku_df.columns else 0

        # Out-of-stock cost
        oos_cost = self._estimate_oos_cost(sku_df)

        # ROI (ИСПРАВЛЕНО: правильный расчет)
        ...
        # Turnover rate (добавлено)
        if total_investment > 0:
            turnover_rate = total_gmv / total_investment
        else:
            turnover_rate = 0
    ...
    def _estimate_oos_cost(self, sku_df: pd.DataFrame) -> float:
        """
        Оценка стоимости out-of-stock ситуаций (ИСПРАВЛЕНО).
        """
        if 'stock_coverage_days' not in sku_df.columns or 'profit_per_day' not in sku_df.columns:
            return 0.0

        # SKU с низким покрытием (< 5 дней)
        low_stock = sku_df[sku_df['stock_coverage_days'] < 5].copy()

        if low_stock.empty:
            return 0.0

        # Оцениваем упущенную прибыль (ИСПРАВЛЕНО: более реалистичная формула)
        lost_sales_days = (5 - low_stock['stock_coverage_days']).clip(lower=0, upper=5)
        # Предполагаем, что теряем 50% от потенциальных продаж при низком стоке
        lost_profit = (low_stock['profit_per_day'] * lost_sales_days * 0.5).sum()
    ...
    def _calculate_inventory_cost(self, sku_df: pd.DataFrame, time_period_days: int) -> float:
        ...
        if stock_col not in sku_df.columns or 'purchase_price_mean' not in sku_df.columns:
            return 0.0
...
class RewardFunction:
    """
    Функция вознаграждения для RL агента (ИСПРАВЛЕНО).
    """

    def __init__(
        self,
        profit_weight: float = 1.0,
        oos_penalty_weight: float = 0.5,  # Увеличено
        inventory_penalty_weight: float = 0.2,  # Увеличено
        diversity_weight: float = 0.3  # Увеличено
    ):
        """
        Args:
    ...
    def calculate(
        ...
        """
        Вычисление reward (ИСПРАВЛЕНО: улучшенные веса).
        """
        # 1. Profit component
        ...
        diversity_reward = diversity_delta * self.diversity_weight * 100
